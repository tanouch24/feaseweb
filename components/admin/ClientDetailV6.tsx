...
export function ClientDetailV6({ clientId }: { clientId: string }) {
  const { data, setClientStatus, setSiteStatus, setSitePreview, addClientNote, inviteClient } = useBackoffice();
  ...
  const sendInvitation = async () => { setInviteLoading(true); setInviteError(""); try { await inviteClient(client.id); } catch (error) { setInviteError(error instanceof Error ? error.message : "Invitation impossible."); } finally { setInviteLoading(false); } };
  return <>
    ...
    <div className="admin-detail-grid">
      ...
      <section className="admin-panel"><PanelTitle title="Espace client" /><DetailRows rows={[["Email", client.email], ["Accès", labelMap[client.accessStatus]]]} />{client.accessStatus !== "actif" ? <button className="admin-button" type="button" onClick={() => void sendInvitation()} disabled={inviteLoading}>{inviteLoading ? "Envoi…" : client.accessStatus === "invitation_envoyee" ? "Renvoyer l’invitation" : "Inviter le client"}</button> : <p className="admin-panel-intro">Compte client actif.</p>}{inviteError && <p className="text-sm text-red-700" role="alert">{inviteError}</p>}</section>
