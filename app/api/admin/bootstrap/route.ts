...
const tables = ["prospects", "clients", "sites", "subscriptions", "payments", "modification_requests", "client_updates", "seo_actions", "seo_metrics", "domains", "internal_notes", "activity_log", "project_intakes", "project_access_requirements"] as const;
...
export async function GET() {
  ...
  const orderColumns: Record<string, string> = { activity_log: "occurred_at", client_updates: "activity_date", seo_actions: "date", seo_metrics: "synced_at", domains: "domain", subscriptions: "updated_at" };
  ...
  const optional = new Set(["project_intakes", "project_access_requirements"]);
  const failure = results.find(([table, result]) => result.error && !optional.has(table));
